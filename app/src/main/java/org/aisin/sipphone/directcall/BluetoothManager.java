...
public class BluetoothManager extends BroadcastReceiver {
	...
	public BluetoothManager() {
		...
		if (!ensureInit()) {
			Log.w("BluetoothManager tried to init but AisinService not ready yet...");
		}
		instance = this;
	}
	...
	public void initBluetooth() {
		if (!ensureInit()) {
			Log.w("BluetoothManager tried to init bluetooth but AisinService not ready yet...");
			return;
		}
		...
		filter.addAction(Compatibility
	...
	private void startBluetooth() {
		...
		if (mBluetoothAdapter != null && mBluetoothAdapter.isEnabled()) {
			...
			boolean success = mBluetoothAdapter.getProfileProxy(mContext,
	...
	private boolean ensureInit() {
		...
		if (mContext == null) {
			if (AisinService.isReady()) {
				mContext = AisinService.instance().getApplicationContext();
			} else {
				return false;
			}
		}
	...
	public boolean isBluetoothHeadsetAvailable() {
		...
		if (mBluetoothAdapter != null && mBluetoothAdapter.isEnabled()
			...
			if (mBluetoothHeadset != null) {
				...
				Log.d(isHeadsetConnected ? "Headset found, bluetooth audio route available"
						: "No headset found, bluetooth audio route unavailable");
			}
	...
	public void onReceive(Context context, Intent intent) {
		...
		if (Compatibility
			...
			if (state == AudioManager.SCO_AUDIO_STATE_CONNECTED) {
				...
				// AisinManager.getInstance().audioStateChanged(AudioState.BLUETOOTH);
				isScoConnected = true;
			} else if (state == AudioManager.SCO_AUDIO_STATE_DISCONNECTED) {
				Log.d("Bluetooth sco state => disconnected");
				// AisinManager.getInstance().audioStateChanged(AudioState.SPEAKER);
